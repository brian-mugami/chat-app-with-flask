...
def home():
    ...
    if request.method == "POST":
        ...
        if not name:
            flash("You must provide a name", category="error")
            return render_template("home.html", code=code, name=name)
        if join != False and not code:
            flash("Please provide a room code", category="error")
            return render_template("home.html", code=code, name=name)
        room = code
        if create != False:
        ...
        flash(message="Welcome to the room", category="success")
...
def room():
    ...
    if room is None or name is None or room not in rooms:
        flash("Create or join a room", category="error")
...
def message(data):
    ...
    send(content, to=room)#sends to js
